...
/*
 * Looks in astr for string in bstr.
...
void do_reboot(USER_DATA *usr, char *argument) {
	...
	for (d = desc_list; d; d = d_next) {
		...
		if (!USR(d) || d->login > CON_LOGIN) {
			...
			fprintf(fp, "%d %s %s %s %s %ld %ld\n", d->descr, usr->pBoard->short_name,
					usr->name, d->ident, d->host, d->rows, d->cols);
			save_user(usr);
		}
	}
...
void copyover_recover(void) {
	...
	for (;;) {
		fscanf(fp, "%d %s %s %s %s %ld %ld\n", &desc, board, name, user, host, &rows, &cols);
		if (desc == -1)
			break;
