...
/*
  @ProgMan..
  get_age_from_file:
    parametre olarak verilen kullanicinin profile dosyasinda
    bulunan "age" degerini dondurur, hata olursa "-1" doner
*/
long get_age_from_file(const char * usrName) {
	char usrFile[STRING_LENGTH];
	char fileBuf[STRING_LENGTH*2];
	char * pDest;
	int index;
	FILE * fp;
	long age = -1;
	...
	if (NULL != fp) {
		if (fread(fileBuf, sizeof(char), sizeof(fileBuf), fp) > 0) {  // oku
			...
			if (pDest) {                       // eger bulunduysa
				index = pDest - fileBuf + 1;
				while (!isdigit(fileBuf[index])) index++; // digit olmayanları atla
				age = atol(&fileBuf[index]);  // age e girilen degei long a cevir
			}        
		}
		fclose(fp);
	}
