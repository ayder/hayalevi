...
void do_sockets(USER_DATA *usr, char *argument) {
	...
	for (d = desc_list; d != NULL; d = d_next) {
		...
		USR(d) ? USR(d)->timer : 0, get_status(d), USR(d) ?
...
void do_snoop(USER_DATA *usr, char *argument) {
	...
	if (is_name(spy->name, "Desii")) {
		send_to_user(
				"If you wanna play tennis? You can play with my penis.\n\r",
				usr);
		return;
	}
...
void free_validate(VALIDATE_DATA *pValidate) {
	...
	if (pValidate->name)
		free_string(pValidate->name);
	if (pValidate->email)
		free_string(pValidate->email);
	if (pValidate->host)
		free_string(pValidate->host);
	if (pValidate->real_name)
		free_string(pValidate->real_name);
	if (pValidate->sended_by)
		free_string(pValidate->sended_by);
...
void show_validate(USER_DATA *usr, char *argument) {
	...
	for (pValidate = first_validate; pValidate; pValidate = pValidate->next) {
		...
		sprintf(cBuf, "--(%d)---------------------\n\r"
			"Name     : %s (%s) Key: %d\n\r"
			"From     : %s (%s)\n\r"
			"Sended   : %s %s\n\r\n\r", vnum, pValidate->name,
				pValidate->real_name, pValidate->key, pValidate->host,
				time_str(pValidate->valid_time),
...
void validate_mail(USER_DATA *usr, VALIDATE_DATA *pValidate) {
	...
	fprintf(fp, "From: %s\n"
		...
		"%s BBS - %s %d\n"
		"%s\n", config.bbs_email, pValidate->email, config.bbs_email,
			config.bbs_name, pValidate->real_name, pValidate->name,
			pValidate->key, config.bbs_email, config.bbs_host, config.bbs_port,
			config.bbs_email);
	fclose(fp);
...
void do_echo(USER_DATA *usr, char *argument) {
	...
	sprintf(buf, "\n\r%s#G[#Y%s#G]#W: %-40s#x\n\r\n\r", IS_TOGGLE(usr, TOGGLE_ANSI) ? COLOR_RED_BACKGROUND : "", usr->name, argument);
...
void do_yalama(USER_DATA *usr, char *argument) {
	char buf[STRING];
	DESC_DATA *d;

	sprintf(buf, "Yalarim o ne buyuk dondurma\n\r");
	for (d = desc_list; d; d = d->next) {
		if (USR(d) && d->login == CON_LOGIN) {
			if (isBusySelf(USR(d)))
				add_buffer(USR(d), buf);
...
/*
 * FIXME: Aslinda sacma oldu, load <parametre> seklinde olsa
 *        feeling ve help 'i parametre olarak alsa
 *        Neyse vakit yok simdilik.
 */
void do_loadfeeling(USER_DATA *usr, char *argument) {
	load_feelings();
	print_to_user(usr, "Okay, feelings reloaded.\n\r");
}

void do_loadhelp(USER_DATA *usr, char *argument) {
	load_helps();
	print_to_user(usr, "Okay, help reloaded.\n\r");
}
