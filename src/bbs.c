...
const char do_naws[] = { IAC, DO, TELOPT_NAWS, '\0' };
...
bool isTrChar(char ch) {
	return (ch == 'ð' || ch == 'ü' || ch == 'þ' || ch == 'ý' || ch == 'ö' || ch
			== 'ç' || ch == 'Ð' || ch == 'Ü' || ch == 'Þ' || ch == 'Ý' || ch
			== 'Ö' || ch == 'Ç');
}
...
int init_socket(int port) {
...
#if defined(SO_DONTLINGER) && !defined(SYSV)
	{
		...
		if (setsockopt(fd, SOL_SOCKET, SO_DONTLINGER, (void *) &ld, sizeof(ld))
				< 0) {
...
void bbs_loop(int control) {
	...
	while (!bbs_down) {
		...
		for (d = desc_list; d != NULL; d = d_next) {
			...
			if ((d->login == CON_LOGIN || USR(d)) && d->ifd != -1 && FD_ISSET(d->ifd, &in_set))
				process_resolve(d);
			...
			if (d->incomm[0] != '\0') {
				...
				if (d->showstr_point) {
					...
					switch (EDIT_MODE(USR(d))) {
						...
						case EDITOR_NONE:
							if (USR(d)->pBoard == board_lookup("chat", FALSE))
		...
		for (d = desc_list; d != NULL; d = d_next) {
			...
			if ((d->fcommand || d->outtop > 0) && FD_ISSET(d->descr, &out_set)) {
...
#define FNDELAY O_NDELAY
	...
	write_to_buffer(dnew, do_naws, 0);
	return;
}
...
void read_from_buffer(DESC_DATA *d) {
	...
	// do telnet options handling here
	i = 0;
	for (;;) {
		if ((d->inbuf[i] & 0xff) == IAC) {
			...
			switch(cmd) {
				...
				case SB:
					if (opt == TELOPT_NAWS) {
						d->cols = (d->inbuf[++i] & 0xff) << 8;
						d->cols |= (d->inbuf[++i] & 0xff);
					...
					// TODO: Handle RFC 2066 - Telnet Charset Option
					break;
			}
			d->inbuf[0] = '\0';
			continue;
...
char* colorize(USER_DATA* usr, char* to) {
	...
	if (is_friend(usr, to)) {
		color = "#C";
	} else if (is_notify(usr, to)) {
		color = "#G";
	} else if (is_enemy(usr, to)) {
		color = "#R";
	} else if (is_ignore(usr, to, TRUE)) {
		color = "#M";
	}
	sprintf(buf, "%s%s#w", color, to);
	return strdup(buf);
}
				...
				case EDITOR_NONE:
					...
					if (unread_notes(usr, usr->pBoard) > 0)
						sprintf(buf, "#x[#Y%s #x(#R%d #Ynew#x)] ",
								usr->pBoard->long_name, unread_notes(usr,
										usr->pBoard));
					else
						sprintf(buf, "#x[#Y%s#x] ", usr->pBoard->long_name);
	...
	switch (d->login) {
			...
			switch (*argument) {
				default:
					for (d_old = desc_list; d_old != NULL; d_old = d_next) {
						...
						if (d_old != d && USR(d_old) && d_old->login != CON_GET_NAME
		...
		case CON_GET_NAME:
			...
			if (fOld) {
				...
				if (usr && (check_banishes(usr, BANISH_USER) || check_banishes(
						usr, 
						BANISH_SITE))) {
					close_socket(d);
					return;
		...
		case CON_GET_EMAIL:
			...
			if (!str_cmp(host, "hotmail.com") || !str_cmp(host, "mailcity.com")
					|| !str_cmp(host, "yahoo.com") || !str_cmp(host,
					"infoseek.com") || !str_cmp(host, "iname.com") || !str_cmp(
					host, "writeme.com") || !str_cmp(host, "usa.net")
...
bool legal_name(char *name) {
	if (is_name_full(name,
			"god Allah sik yarrak fuck fucker dick suck deneme ege"))
		return FALSE;

	if (is_name_full(name,
			"fuckyou pussy amcik bbs root admin sysop operator gehenna bilmuh imrahil"))
		return FALSE;
	...
	if (is_name_full(name,
			"dassak dasak yarak shit butt ass asshole bastard ghenna"))
		return FALSE;

	if (str_cmp(capitalize(name), "Baxter") && (!str_prefix("bax", name)
			|| !str_suffix("baxter", name)))
		return FALSE;

	if (str_cmp(capitalize(name), "Neruda") && (!str_prefix("neru", name)
			|| !str_suffix("neruda", name)))
		return FALSE;

	if (str_cmp(capitalize(name), "Mtb") && (!str_prefix("mtb", name)
			|| !str_suffix("mtb", name)))
		return FALSE;

	if (str_cmp(capitalize(name), "Swoop") && (!str_prefix("swo", name)
			|| !str_suffix("swoop", name)))
		return FALSE;
...
bool check_reconnect(DESC_DATA *d, char *name, bool fConn) {
	...
	for (usr = user_list; usr != NULL; usr = usr->next) {
		if ((!fConn || usr->desc == NULL) && !str_cmp(USR(d)->name, usr->name)) {
			if (fConn == FALSE) {
				if (USR(d)->password)
...
void get_small_host(USER_DATA *usr) {
	...
	if (!str_cmp(host_name, "localhost") || !str_cmp(host_name, "<unknown>")) {
		buf[0] = '\0';
