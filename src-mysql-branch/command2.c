		...
		if (!str_prefix(USR(d)->alias[alias], argument)) {
			...
			if (!str_cmp(USR(d)->alias[alias], name)) {
				...
				if (strlen(buf) > INPUT_LENGTH - 1) {
					send_to_user("Alias substitution too long. Truncated.\r\n", 
					USR(d));
		...
		case 'E':
			...
			if (!str_cmp(word, "End")) {
				...
				sprintf(
						buf,
						"#c*%s* %s%-12s #W(#x%s#W)#x\n\r",
						!Validated ? " NEW " : level == ADMIN ? "ADMIN" : "BBS",
						is_friend(usr, name) ? "#C"
								: is_notify(usr, name) ? "#G" : is_enemy(usr,
										name) ? "#R" : "#Y", name, title);
				...
				if (name)
					free_string(name);
				if (real_name)
					free_string(real_name);
				if (title)
					free_string(title);
				if (email)
					free_string(email);
				if (alt_mail)
					free_string(alt_mail);
				if (host_name)
					free_string(host_name);
				if (host_small)
					free_string(host_small);
				if (url)
					free_string(url);
				if (plan)
					free_string(plan);
...
void do_finger(USER_DATA *usr, char *argument) {
	...
	if (IS_ADMIN(usr)) {
		print_to_user(usr, "#cTotal login: #C%d#c, "
			"and have left #C%d #cnotes.#x (Admin Only)\n\r",
...
void cmd_chat_talk(USER_DATA *usr, char *argument) {
	...
	for (d = desc_list; d; d = d->next) {
		...
		&& USR(d)->pBoard == board_lookup("chat", FALSE)) {
			...
			is_enemy(USR(d), usr->name) ? "#R" : "#x", usr->name, argument);
