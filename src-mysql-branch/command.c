 ...
 * Command table.
 */
const struct cmd_type cmd_table [] = { { "age", do_age, 0, 1, 1 }, { "alia",
		do_alia, 0, 0, 0 }, { "alias", do_alias, 0, 1, 0 }, { "anonnote",
		do_anonnote, 0, 1, 0 }, { "bug", do_bug, 0, 1, 0 }, { "cls", do_cls, 0,
		1, 1 }, { "clip", do_clip, 0, 1, 0 },
		{ "credits", do_credits, 0, 1, 0 }, { "date", do_time, 0, 1, 1 }, {
				"enemy", do_enemy, 0, 1, 0 }, { "finger", do_finger, 0, 1, 1 },
		{ "feeling", do_feeling, 0, 1, 0 }, { "fl", do_forumlist, 0, 0, 1 }, {
				"fly", do_jump, 0, 0, 1 }, { "fnotify", do_fnotify, 0, 1, 0 },
		{ "forumlist", do_forumlist, 0, 1, 1 },
		{ "friend", do_friend, 0, 1, 0 }, { "from", do_from, 0, 1, 0 }, { "go",
				do_jump, 0, 0, 1 }, { "help", do_help, 0, 1, 1 }, { "hide",
				do_hide, 0, 1, 0 }, { "info", do_info, 0, 1, 1 }, { "ignore",
				do_ignore, 0, 1, 0 }, { "idea", do_idea, 0, 1, 0 }, { "jump",
				do_jump, 0, 1, 1 }, { "look", do_look, 0, 1, 1 }, { "list",
				do_list, 0, 1, 1 }, { "ln", do_listnew, 0, 1, 1 }, { "listnew",
				do_listnew, 0, 1, 1 }, { "mail", do_mail, 0, 1, 0 }, { "mx",
				do_mx, 0, 1, 0 }, { "new", do_new, 0, 1, 1 }, { "newmsgs",
				do_newmsgs, 0, 1, 1 }, { "next", do_next, 0, 1, 1 }, { "nm",
				do_newmsgs, 0, 0, 1 }, { "note", do_note, 0, 1, 0 }, {
				"notify", do_notify, 0, 1, 0 }, { "nx", do_next, 0, 0, 1 }, {
				"plan", do_plan, 0, 1, 0 },
		{ "previous", do_previous, 0, 1, 1 }, { "profile", do_plan, 0, 0, 0 },
		{ "password", do_passwd, 0, 1, 1 }, { "reply", do_reply, 0, 1, 0 }, {
				"read", do_read, 0, 1, 1 }, { "readall", do_readall, 0, 1, 1 },
		{ "readlast", do_readlast, 0, 1, 1 }, { "remove", do_remove, 0, 1, 0 },
		{ "save", do_save, 0, 1, 1 }, { "set", do_set, 0, 1, 0 }, { "search",
				do_search, 0, 1, 1 }, { "sendclip", do_sendclip, 0, 1, 0 }, {
				"showclip", do_showclip, 0, 1, 0 },
		{ "time", do_time, 0, 1, 1 }, { "title", do_title, 0, 1, 0 }, {
				"toggle", do_toggle, 0, 1, 0 }, { "quit", do_quit, 0, 1, 1 }, {
				"unalias", do_unalias, 0, 1, 0 }, { "unflash", do_unflash, 0,
				1, 1 }, { "unhide", do_hide, 0, 1, 0 }, { "uptime", do_uptime,
				0, 1, 1 }, { "version", do_version, 0, 1, 1 }, { "who", do_who,
				0, 1, 1 }, { "Who", do_Who, 0, 1, 1 }, { "whoami", do_whoami,
				0, 1, 1 }, { "x", do_x, 0, 1, 0 },
		{ "xers", do_xers, 0, 1, 1 }, { "xm", do_xers, 0, 0, 1 }, { "xn",
				do_xers, 0, 0, 1 }, { "zap", do_zap, 0, 1, 0 },

		/*
		 * Moderator commands.
		 */
		{ "foruminfo", do_foruminfo, 1, 2, 0 }, { "kick", do_kick, 1, 2, 0 }, {
				"stats", do_stats, 1, 2, 0 },

		/*
		 * Admin commands.
		 */
		{ "ac", do_admin, 3, 0, 0 }, { "admin", do_admin, 3, 1, 0 }, {
				"banish", do_banish, 3, 1, 0 },
		{ "config", do_config, 3, 1, 0 }, { "deluser", do_deluser, 3, 1, 0 }, {
				"disconnect", do_disconnect, 3, 1, 0 }, { "echo", do_echo, 3,
				1, 0 }, { "yalama", do_yalama, 3, 1, 0 }, { "force", do_force,
				3, 1, 0 }, { "forumset", do_setforum, 3, 1, 0 }, { "hostname",
				do_hostname, 3, 1, 0 }, { "invis", do_invis, 3, 1, 0 }, {
				"memory", do_memory, 3, 1, 0 }, { "reboo", do_reboo, 3, 0, 0 },
		{ "reboot", do_reboot, 3, 1, 0 }, { "saveall", do_saveall, 3, 1, 0 }, {
				"setuser", do_userset, 3, 1, 0 }, { "shutdow", do_shutdow, 3,
				0, 0 }, { "shutdown", do_shutdown, 3, 1, 0 }, { "sockets",
				do_sockets, 3, 1, 0 }, { "snoop", do_snoop, 3, 1, 0 }, {
				"unbanish", do_unbanish, 3, 1, 0 }, { "userset", do_userset, 3,
				1, 0 }, { "validate", do_validate, 3, 1, 0 }, { "unkick",
				do_unkick, 3, 1, 0 }, { "addforum", do_addforum, 3, 1, 0 }, {
				"delforum", do_delforum, 3, 1, 0 }, { "setforum", do_setforum,
				3, 1, 0 }, { "showmods", do_showmods, 3, 1, 0 }, { "statforum",
				do_statforum, 3, 1, 0 },

		{ "banishes", do_banishes, 0, 1, 0 },

		{ "test", do_test, 3, 1, 0 }, { "rtest", do_rtest, 3, 1, 0 },
...
void process_command(USER_DATA *usr, char *argument) {
	...
	if (!usr->Validated && cmd_table[cmd].valid == 0) {
		if (is_turkish(usr))
			...
			send_to_user(
					"You are unvalidated user.  You can't use that command.\n\r",
					usr);
		return;
	}
