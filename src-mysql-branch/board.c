...
void do_list(USER_DATA * usr, char *argument) {
	...
	for (pNote = pBoard->first_note; pNote; pNote = pNote->next) {
		...
		char* color = "#Y";
		if (is_friend(usr, pNote->sender)) {
			color = "#C";
		} else if (is_notify(usr, pNote->sender)) {
			color = "#G";
		} else if (is_enemy(usr, pNote->sender)) {
			color = "#R";
		}
		sprintf(buf, "%3d #m##%-6ld %s%-12s  #g%-26s #y%s#x\n\r", vnum,
				pNote->vnum, color, pNote->sender, pNote->subject, pNote->date);
		add_buf(output, buf);
		found = TRUE;
	}
...
void do_new(USER_DATA * usr, char *argument) {
	...
	for (; pBoard; pBoard = pBoard->next) {
		...
		// grab next new note from forum
		vnum = 1;
		pNote = find_next_new(usr, pBoard, &vnum);
		if (pNote) {
			if (usr->pBoard != pBoard) {
				sprintf(buf, "%s ", pBoard->short_name);
				if (is_name_full(buf, usr->zap)) {
			...
			sprintf(buf, "%d #m##%ld  %s%s  #y%s, #GSubject: #x%s#x\n\r\n\r",
					vnum, pNote->vnum, color, pNote->sender, pNote->date,
					pNote->subject);
			add_buf(output, buf);
			add_buf(output, pNote->text);
			add_buf(output, "#x");
		...
		if (!str_cmp(pKick->name, usr->name) && pKick->duration >= current_time) {
			if (fMessage) {
				sprintf(buf, "You are kicked due to %s\n\r"
					"Removal date: %s\n\r", pKick->reason, get_age(remove, 
				FALSE));
				send_to_user(buf, usr);
			}
			return TRUE;
		} else if (!str_cmp(pKick->name, usr->name) && pKick->duration
				< current_time && fMessage) {
...
void do_kick(USER_DATA * usr, char *argument) {
	...
	if (arg1[0] == '\0' || arg2[0] == '\0' || argument[0] == '\0' || (IS_ADMIN(usr) && arg3[0] == '\0')) {
		if (IS_ADMIN(usr))
...
void do_Who(USER_DATA * usr, char *argument) {
	...
	for (d = desc_list; d; d = d->next) {
		...
		IS_TOGGLE(USR(d), TOGGLE_XING) ? " " : "*", !USR(d)->Validated ? "#D" : is_friend(usr, USR(d)->name) ? "#C" : is_notify(usr, USR(d)->name) ? "#G" : is_enemy(usr, USR(d)->name) ? "#R" : "#Y", 
