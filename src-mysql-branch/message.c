...
void do_xing(USER_DATA *usr, char *argument, bool fXing) {
	...
	sprintf(buf, "You have %d lines.  Please type <CR> twice to send.\n\r",
			MAX_X_LINES);
	send_to_user(buf, usr);
			...
			if (isBusy(usr)) {
				...
				sprintf(output,
						"\n\rThese eXpress messages are held while you were busy."
							...
							"\n\r%s#x\n\r", IS_TOGGLE(xing, TOGGLE_BEEP) ? "\007" : "", usr->name,
						usr->xing_time, usr->msg);
				add_buffer(xing, output);
...
void send_feeling(USER_DATA *usr, int type) {
	...
	case FEELING_POKE:
		feeling = "#Woooooooooo     ooooooo    oooo   oooo  ooooooooooo\n\r"
	...
	if (isBusy(usr)) {
		...
		sprintf(
				...
				IS_TOGGLE(xing, TOGGLE_BEEP) ? "\007" : "", usr->name,
				usr->xing_time, feeling);
